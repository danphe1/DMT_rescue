...
self.addEventListener('push',event=>{
  ...
  const options={body,tag:safe?'rescue-safe-due':'rescue-4h-checkin',renotify:true,requireInteraction:true,vibrate:[500,250,500,250,900],data:{url:d.url||'/rescuer',kind},actions:[{action:safe?'safe':'ack',title:safe?'I AM SAFE / म सुरक्षित छु':'Acknowledge / स्वीकार'},{action:'open',title:'Open Portal / पोर्टल खोल्नुहोस्'}]};
  event.waitUntil(self.registration.showNotification(title,options));
});
self.addEventListener('notificationclick',event=>{
  const action=event.action||'open';const data=event.notification.data||{};event.notification.close();
  const base=data.url||'/rescuer';const target=base+(base.includes('?')?'&':'?')+'notification_action='+encodeURIComponent(action);
